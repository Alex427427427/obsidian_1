...
where $\rho$ is the linear voltage-to-displacement calibration factor, $k_B$ is Boltzmann's constant, $\beta$ is the Stokes drag of the sphere which is equivalent to the effective inductance or inertia of the system, $f_0$ is the corner frequency [5]. 
...
### 3.5 Experiment 2: Measuring the transverse spring constant at different depths
